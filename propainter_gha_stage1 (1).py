...
# CATATAN: audio TIDAK diekstrak di sini (beda dari versi sebelumnya).
# Pipeline besar (job final-video) SELALU mengganti audio dengan narasi
# TTS (-map 1:a:0) dan tidak pernah memakai audio asli dari tahap ini,
# jadi mux audio di sini cuma kerja sia-sia -- dihapus untuk mengurangi
# satu titik gagal lagi & mempercepat Stage 1.
...
# =====================================================================
# EXPORT HASIL SEBAGAI 2 VIDEO KOMPAK (bukan lagi lanjut Stage 2 di
# Colab). Blending + render final sekarang dikerjakan di runner GHA
# sendiri (murni CPU, tidak butuh GPU sama sekali) -- supaya sesi Colab
# ini bisa langsung ditutup begitu ProPainter selesai, tanpa perlu
# tetap hidup lebih lama lagi untuk tahap berikutnya. Ini untuk
# menghindari sesi mati/di-revoke (404/401) di tengah proses seperti
# yang pernah terjadi ketika Stage 2 masih dijalankan di Colab juga.
#
#   1) propainter_inpainted.mp4 -- hasil ProPainter (COMBINED_DIR),
#      di-encode h264 kualitas tinggi. Resolusinya MASIH resolusi kerja
#      ProPainter (pp_width x pp_height), BUKAN resolusi asli -- akan
#      di-upscale saat blending di GHA (sama seperti alur lama).
#   2) propainter_masks.mkv -- mask (MASKS_DIR), di-encode LOSSLESS
#      (codec "png" per-frame) karena mask harus tetap biner 0/255
#      persis, tidak boleh ada artefak kompresi lossy. Ukurannya kecil
#      karena isinya mayoritas hitam polos.
#   3) stage1_meta.json -- fps/width/height/frame_idx/BASENAME, supaya
#      GHA tidak perlu menebak-nebak nilai ini.
#
# Original frame TIDAK perlu dikirim -- GHA sudah punya video_raw.mp4
# yang identik (artifact yang sama persis yang di-upload ke Colab ini),
# jadi original frame akan diekstrak ulang di sana secara lokal.

print("\nMerender 2 video kompak untuk dikirim ke GHA...")

INPAINTED_VIDEO = "/content/propainter_inpainted.mp4"
MASKS_VIDEO = "/content/propainter_masks.mkv"
META_PATH = "/content/stage1_meta.json"

run_checked([
    "ffmpeg", "-y",
    "-framerate", str(fps),
    "-i", os.path.join(COMBINED_DIR, "%05d.png"),
...
print(f"  {INPAINTED_VIDEO}: {os.path.getsize(INPAINTED_VIDEO)} bytes")
print(f"  {MASKS_VIDEO}: {os.path.getsize(MASKS_VIDEO)} bytes")
